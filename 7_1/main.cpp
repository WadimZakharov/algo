...
private:
	...
	int _addnode(TreeNode *node, int value, int priority);
	void _deletenode(TreeNode **node, int index);
	void fix_count(TreeNode* &node);
};

void Treap::fix_count(TreeNode* &node)
{
	node->count = 1;
	if (node->left) 
...
{
	if (root)
	_deletenode(&root, index);
}

void Treap::_deletenode(TreeNode **node, int index) {
	TreeNode *_node = (*node);
	int left_count = 0;
	if (_node->left)
	...
	{
		_deletenode(&_node->left, index);
		fix_count(_node);
	}
	else 
	{
		_deletenode(&_node->right, index - left_count - 1);
		fix_count(_node);
	}
}
...
int Treap::AddNode(int val, int priority) 
{
	...
	{
		return _addnode(root, val, priority);
	}
}

int Treap::_addnode(TreeNode *node, int value, int priority)
{
	TreeNode **_node;
	int position = 0;
	if (value > node->value)
	...
	{
		position += _addnode(*_node, value, priority);
		fix_count(node);
		return position;
	}
...
int main() {
	...
	for(int i=0; i<n; i++)
	{
		...
		if (comand == 1)
		{
			int position = treap.AddNode(value, rand());
			cout << position << endl;
		}
		else
		{
